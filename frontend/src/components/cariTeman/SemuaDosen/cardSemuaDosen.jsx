...
const CardSemuaDosen = ({ alldosen }) => {
  const topikList = alldosen.Topik.split(',');
  return (
          ...
          <div className='bg-gray-50 border-1 border-gray-200 rounded-xl shadow-sm text-left p-2 w-full h-full relative flex flex-col justify-between'>
            <div className="flex flex-col items-center h-full">
              ...
              <img
                ...
                alt=""
              />
              <p className='font-bold text-m text-center'>{alldosen.Nama}</p>
              <p className='text-xs text-center text-gray-500'>{alldosen.PerguruanTinggi}</p>
              <p className='text-xs text-center text-gray-500'>{alldosen.Kota}, {alldosen.Provinsi}</p>
            ...
            <div className='flex py-1 text-gray-500'>
                <p className='text-xs ml-2 mr-10'>{alldosen.Bergabung}</p>
                <p className='text-xs'>{alldosen.Likes}</p>
            </div>
          </div>
        </li>
      </ul>
