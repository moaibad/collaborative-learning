...
const LoginPage = ({ onLogin }) => {
  ...
  const login = useGoogleLogin({
    onSuccess: (codeResponse) => {
      cookies.set("user_token", codeResponse["access_token"], {
        path: "/",
        maxAge: 3600,
      });
      setUser(codeResponse);
      ...
      axios
        .post(`${VITE_BACKEND_CTB_URL}/status`, {
          user_token: codeResponse["access_token"],
        })
        .then((res) => {
    ...
    <>
      <body class="bg-orange-100 min-h-screen">
        <div className="flex pt-5 px-10 pb-5">
          ...
          <h1 className="font-extrabold text-4xl">Colle</h1>
        ...
        <p className="flex justify-center text-2xl font-bold my-10">
          Login to Account
        </p>
